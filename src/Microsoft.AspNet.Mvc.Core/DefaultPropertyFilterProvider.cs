...
{
    /// <summary>
    /// Default implementation for <see cref="IModelPropertyFilterProvider"/>.
    /// Provides a expression based way to provide include properties.
    /// </summary>
    /// <typeparam name="TModel">The target model Type.</typeparam>
    public class DefaultModelPropertyFilterProvider<TModel> : IModelPropertyFilterProvider
        where TModel : class
    {
        private Func<ModelBindingContext, string, bool> _defaultFilter = (context, propertyName) => true;
